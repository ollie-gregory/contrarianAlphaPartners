...
def manager_view(user):
    ...
    with col1:
        ...
        with pool.connect() as conn:
            total_value = conn.execute(sqlalchemy.text(query)).fetchone()[0]
    ...
    with col2:
        ...
        df1 = df.pivot_table(index=None, columns="Asset", values="Asset Value", aggfunc='first') # Pivot the dataframe
        
        # Top performing stocks widget:
        query = f"""
...
def ceo_view(user):
    ...
    with col2:
        ...
        with pool.connect() as conn:
            regions_df = pd.read_sql_query(query, conn)
        ...
        # Firm investments by region widget:
        query = """
                SELECT 
                    s.ticker AS "Ticker",
    ...
    with col3:
        ...
        for i in range(10):
            ...
            with pool.connect() as conn:
                name = conn.execute(sqlalchemy.text(query)).fetchone()[0]
